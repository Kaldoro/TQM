...
int unitSize (loc projectLoc) {
	...
	// Severity (0,1,2,3,4) corresponds to (★ , ★★ , ★★★ , ★★★★, ★★★★★)
	if (perSeverity[1] > 54.0 || perSeverity[2] > 43.0 || perSeverity[3] > 24.2) return 0;
	if (perSeverity[1] > 35.4  || perSeverity[2] > 25.0   || perSeverity[3] > 14.0)  return 1;
	if (perSeverity[1] > 27.6 || perSeverity[2] > 16.1 || perSeverity[3] > 7.0)  return 2;
	if (perSeverity[1] > 12.3   || perSeverity[2] > 6.1  || perSeverity[3] > 0.8)   return 3; 
	return 4;
...
tuple[int, int] countLinesAndCodeLines(loc projectLoc){
	strs = projectToListString(projectLoc);
